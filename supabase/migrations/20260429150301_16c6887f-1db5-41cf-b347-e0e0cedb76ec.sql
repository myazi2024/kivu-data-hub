-- 1) Index sur les FK pointant vers cadastral_parcels (cause principale du timeout DELETE)
CREATE INDEX IF NOT EXISTS idx_mortgages_parcel_id
  ON public.cadastral_mortgages(parcel_id);
CREATE INDEX IF NOT EXISTS idx_boundary_history_parcel_id
  ON public.cadastral_boundary_history(parcel_id);
CREATE INDEX IF NOT EXISTS idx_land_disputes_parcel_id
  ON public.cadastral_land_disputes(parcel_id);
CREATE INDEX IF NOT EXISTS idx_mutation_requests_parcel_id_fk
  ON public.mutation_requests(parcel_id);
CREATE INDEX IF NOT EXISTS idx_expertise_requests_parcel_id_fk
  ON public.real_estate_expertise_requests(parcel_id);
CREATE INDEX IF NOT EXISTS idx_subdivision_requests_parcel_id_fk
  ON public.subdivision_requests(parcel_id);

-- 2) Étendre le statement_timeout de la RPC nettoyage par chunk
CREATE OR REPLACE FUNCTION public._cleanup_test_data_chunk_internal(
  p_step text,
  p_limit integer
)
RETURNS integer
...
DECLARE
  v_deleted int := 0;
BEGIN
  -- Plus de marge pour absorber les gros lots (FK checks + cascades)
...
-- 3) Purge automatique des jobs de génération orphelins (heartbeat > 3 min)
CREATE OR REPLACE FUNCTION public._purge_stale_test_generation_jobs()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  WITH stale AS (
    UPDATE public.test_generation_jobs
    SET status = 'error',
        error = COALESCE(error, 'Heartbeat perdu (job orphelin auto-purgé)'),
...
-- Exécutable par les admins authentifiés (et par le service_role)
REVOKE ALL ON FUNCTION public._purge_stale_test_generation_jobs() FROM public;
GRANT EXECUTE ON FUNCTION public._purge_stale_test_generation_jobs() TO authenticated, service_role;
-- Fix 1: Add 'returned' to status CHECK constraint
ALTER TABLE cadastral_contributions DROP CONSTRAINT IF EXISTS cadastral_contributions_status_check;
ALTER TABLE cadastral_contributions ADD CONSTRAINT cadastral_contributions_status_check 
  CHECK (status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text, 'returned'::text]));

-- Fix 2: Update trigger to match DB default contribution_type 'new' instead of 'nouveau'
CREATE OR REPLACE FUNCTION create_parcel_from_approved_contribution()
RETURNS TRIGGER AS $$
BEGIN
  -- Si la contribution vient d'être approuvée
  -- Accept both 'new' (DB default) and 'nouveau' (legacy) for backward compatibility
  IF NEW.status = 'approved' AND OLD.status IN ('pending', 'returned') AND NEW.contribution_type IN ('new', 'nouveau') THEN
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO cadastral_parcels (
        ...
        NEW.groupement, NEW.village, NEW.circonscription_fonciere,
      ...
      -- Copier les historiques si présents (keys are now snake_case from front-end)

...
DECLARE
  ...
  v_pn text;
BEGIN
  v_bypass := current_setting('app.test_cleanup_in_progress', true);
  IF v_bypass = '1' THEN
    IF TG_OP = 'DELETE' THEN
      BEGIN v_pn := (to_jsonb(OLD) ->> 'parcel_number'); EXCEPTION WHEN OTHERS THEN v_pn := NULL; END;
    ELSE
      BEGIN v_pn := (to_jsonb(NEW) ->> 'parcel_number'); EXCEPTION WHEN OTHERS THEN v_pn := NULL; END;
    END IF;
    IF v_pn IS NOT NULL AND v_pn LIKE 'TEST-%' THEN
      IF TG_OP = 'DELETE' THEN RETURN OLD; ELSE RETURN NEW; END IF;
    END IF;
  END IF;

  IF TG_OP = 'INSERT' THEN
    PERFORM public.log_audit_action('INSERT', TG_TABLE_NAME, NEW.id, NULL, to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'UPDATE' THEN
    PERFORM public.log_audit_action('UPDATE', TG_TABLE_NAME, NEW.id, to_jsonb(OLD), to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    PERFORM public.log_audit_action('DELETE', TG_TABLE_NAME, OLD.id, to_jsonb(OLD), NULL);
    RETURN OLD;
  END IF;
  RETURN NULL;
END;
...
DECLARE
  v_deleted int := 0;
BEGIN
  PERFORM set_config('app.test_cleanup_in_progress', '1', true);

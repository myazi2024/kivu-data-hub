...
Deno.serve(async (req) => {
  ...
  const rl = await enforceRateLimit(req, "payment.test_provider");
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: false, message: (err instanceof Error ? err.message : String(err)) || "Erreur interne" }),

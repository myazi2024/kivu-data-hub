...
export const ExpertiseBlock: React.FC<Props> = ({ data }) => {
  ...
  const byCommune = useMemo(() => countBy(filtered.filter(r => r.parcel_type === 'SU' || r.section_type === 'urbaine'), 'commune'), [filtered]);
  const byTerritoire = useMemo(() => countBy(filtered.filter(r => r.parcel_type === 'SR' || r.section_type === 'rurale'), 'territoire'), [filtered]);
  const byCollectivite = useMemo(() => countBy(filtered.filter(r => r.parcel_type === 'SR' || r.section_type === 'rurale'), 'collectivite'), [filtered]);

  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Card className="bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-950/30 dark:to-orange-900/20">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">Total demandes</p>
            <p className="text-2xl font-bold text-orange-700 dark:text-orange-400">{filtered.length}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/30 dark:to-amber-900/20">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-amber-700 dark:text-amber-400">{filtered.filter(r => r.status === 'pending').length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><ArrowRightLeft className="h-4 w-4 text-orange-500" /> Statut des demandes</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Type de mutation</CardTitle></CardHeader>
          <CardContent>{byType.length === 0 ? <NoData /> : (
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={byType} layout="vertical" margin={{ left: 20 }}>
                ...
                <Tooltip /><Bar dataKey="value" fill={CHART_COLORS[6]} radius={[0, 4, 4, 0]} name="Demandes" />
              </BarChart>
            </ResponsiveContainer>
          )}</CardContent>
        </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={byProvince.slice(0, 10)}>
                  ...
                  <Tooltip /><Bar dataKey="value" fill={CHART_COLORS[6]} radius={[4, 4, 0, 0]} name="Demandes" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <Card className="md:col-span-2">
          <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><TrendingUp className="h-4 w-4 text-blue-500" /> Évolution des demandes de mutation</CardTitle></CardHeader>
          <CardContent>{trend.length === 0 ? <NoData /> : (
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={trend}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis />
                <Tooltip /><Area type="monotone" dataKey="value" stroke={CHART_COLORS[6]} fill={CHART_COLORS[6]} fillOpacity={0.15} name="Demandes" />
              </AreaChart>
            </ResponsiveContainer>
          )}</CardContent>
        </Card>

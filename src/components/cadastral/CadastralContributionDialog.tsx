      ...
      <Dialog open={open} onOpenChange={state.handleAttemptClose}>
        <DialogContent
          ...
          onInteractOutside={(e) => {
            ...
            if (target.closest('[data-whatsapp-button="true"]')) e.preventDefault();
          }}
        >
          ...
          <Tabs value={state.activeTab} className="w-full" onValueChange={state.handleTabChange}>
            <div className="sticky top-0 z-20 bg-background px-2 sm:px-4 pt-2 pb-1.5 border-b shadow-sm">
              <TabsList className="grid w-full grid-cols-5 h-10 bg-muted/50 p-0.5 rounded-xl shadow-inner gap-0.5">
                ...
                <TabsTrigger value="location" disabled={!state.isTabAccessible('location')} className="data-[state=active]:bg-background data-[state=active]:shadow-md transition-all text-sm font-semibold py-1.5 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed">Lieu</TabsTrigger>
                ...
                <TabsTrigger value="obligations" disabled={!state.isTabAccessible('obligations')} className="data-[state=active]:bg-background data-[state=active]:shadow-md transition-all text-sm font-semibold py-1.5 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed">Taxes</TabsTrigger>

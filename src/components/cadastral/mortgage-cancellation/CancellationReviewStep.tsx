...
import { FileCheck, MapPin, Building, User, FileText, Upload, DollarSign, ExternalLink } from 'lucide-react';
          ...
          <div className="space-y-2">
            {formData.supportingDocuments.map((file, index) => (
              <div key={index} className="flex items-center justify-between rounded-lg border p-2">
                <p className="text-xs text-muted-foreground flex items-center gap-1 truncate min-w-0"><FileText className="h-3 w-3" />{file.name}</p>
                <Button type="button" variant="ghost" size="sm" className="h-7 text-xs gap-1" onClick={() => openPreview(file)}>

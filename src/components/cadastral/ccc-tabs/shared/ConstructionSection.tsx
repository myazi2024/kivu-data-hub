        ...
        {permitMode === 'existing' && (
          <div className="space-y-4 animate-fade-in">
            {buildingPermits.map((permit, index) => (
              <div key={(permit as { id?: string }).id ?? `permit-idx-${index}`} className={cn("border-2 rounded-2xl p-4 space-y-4 bg-card shadow-md", highlightIncompletePermit && index === buildingPermits.length - 1 && (!permit.permitNumber || !permit.issueDate) ? 'ring-2 ring-primary border-primary animate-pulse' : 'border-border')}>

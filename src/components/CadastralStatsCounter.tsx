  ...
  if (loading) {
    return (
      <div className="w-full max-w-4xl mx-auto mt-4 px-4">
        <div className="bg-white/90 backdrop-blur-sm rounded-lg border border-white/20 shadow-sm">
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-lg border border-white/20 shadow-sm">
        <div className="px-4 py-3">
          ...
          <div className="flex items-center justify-center gap-6 text-sm">
            ...
            {/* Séparateur */}
            <div className="w-px h-4 bg-seloger-dark/20"></div>
              ...
              {stats.map((stat, index) => {
                ...
                return (
                  <div key={stat.serviceId} className="flex items-center gap-1">
                    ...
                    <span className="text-seloger-dark/60 text-xs hidden sm:inline">{shortName}:</span>
                    <span className="font-medium text-seloger-red text-xs">{stat.count}</span>
                    {index < stats.length - 1 && <span className="text-seloger-dark/30 mx-1">•</span>}
                  </div>
                );
              })}
            </div>

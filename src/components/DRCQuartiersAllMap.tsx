...
interface Feature {
  type: string;
  properties: { name: string; commune?: string; ville?: string; [k: string]: any };
...
const DRCQuartiersAllMap: React.FC<Props> = ({
  ville,
  commune,
  quartier,
  showAll,
  ...
  const features = useGeoJsonData<Feature>('/drc-quartiers.geojson');
  ...
  const filtered = useMemo(() => {
    ...
    return features.filter(f => {
      if (ville && norm(f.properties.ville) !== norm(ville)) return false;
      if (commune && norm(f.properties.commune) !== norm(commune)) return false;
      return true;
    });
  }, [features, ville, commune]);
  ...
  if (!showAll && !ville) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground text-[10px]">
        Sélectionnez une ville pour afficher les quartiers
      </div>
    );
  }
  ...
  if (filtered.length === 0 && features.length > 0) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground text-[10px]">
        Aucun quartier trouvé{ville ? ` pour « ${ville} »` : ''}
      </div>
    );
  }

  const padding = 20;
  // Hide labels at national scale to avoid DOM overload
  const showLabels = filtered.length <= 30 || !!quartier;

  const hoveredFeature = hovered ? filtered.find(f => f.properties.name === hovered) : null;
    ...
    <div ref={containerRef} className="w-full h-full relative">
      <svg viewBox={`0 0 ${dims.w} ${dims.h}`} className="w-full h-full">
        {filtered.map((f, i) => {
          ...
          const isSelected = quartier && norm(name) === norm(quartier);
      ...
      {hoveredFeature && (
        <div className="absolute top-2 left-2 bg-background/90 backdrop-blur-sm border border-border/50 rounded px-2 py-1 text-[10px] shadow-sm pointer-events-none z-10">
          <span className="font-medium">{hoveredFeature.properties.name}</span>
          <span className="text-muted-foreground ml-1">
            — {hoveredFeature.properties.commune || ''}{hoveredFeature.properties.ville ? `, ${hoveredFeature.properties.ville}` : ''}
          </span>
        </div>
      )}
...
export default DRCQuartiersAllMap;
